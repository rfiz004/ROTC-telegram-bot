...
from admin_province_handler import (
    ...
    run_food_processing, back_to_admin_menu, admin_lock_shop, toggle_block_country, handle_admin_search_shop, handle_admin_search_input
)
...
async def handle_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        # Handle grain management input
            ...
            if await handle_admin_search_input(update, context):
                return
...
# app.add_handler(CallbackQueryHandler(review_item_details, pattern=r"^admin_review_item_[^_]+_[^_]+_.*$"))
...
app.add_handler(CallbackQueryHandler(show_provinces, pattern=r"^country_select_"))
app.add_handler(CallbackQueryHandler(handle_back_buttons, pattern=r"^(country_back|province_back|review_back)$"))
...
app.add_handler(CallbackQueryHandler(show_pending_items, pattern=r"^province_"))
app.add_handler(CallbackQueryHandler(review_item_details, pattern=r"^review_"))
app.add_handler(CallbackQueryHandler(approve_item, pattern=r"^approve_"))
app.add_handler(CallbackQueryHandler(reject_item, pattern=r"^reject_"))
